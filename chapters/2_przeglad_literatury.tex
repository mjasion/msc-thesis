...
Język Go, potocznie nazywany również \textsl{golang}, powstał w 2007 roku w firmie Google. Autorami jego byli Ken Thompson, Robert Griesemer oraz Rob Pike\cite{programmingingo}. Każdy z nich miał swój wkład w rozwój języka \textsl{C}. Chcieli oni stworzyć język programowania, który będzie w momencie tworzenia tym, czym \textsl{C} był w latach 80. Miał pozwalać na tworzenie dużych systemów, które będą się łatwo skalować.

\textsl{Go} był tworzony z myślą o programowaniu na systemy wielowątkowe. Wzorem do tego był język \textsl{Erlang} gdzie zarządzanie wątkami było proste i wykorzystywało równomiernie dostępne rdzenie. Jednak w przypadku środowisk jednowątkowych wydajność aplikacji w \textsl{Erlang} była mniejsza od tej w \textsl{C}. \textsl{Go} miało być uniwersalne. Podstawowym elementem wykorzystywanym do programowania równoległego są \textsl{go rutyny}(ang. \textsl{gorutine})\cite{gophrasebook}.  

Kolejną rzeczą, jaką \textsl{Go} 
...
\begin{itemize}
\item 
\item będzie statycznie typowany - jak w \textsl{Java} i \textsl{C++}
\item będzie wspierał mechanizmy programowania dynamicznego np. inicjalizacje na podstawie inferencji typów
\item kompilacja będzie znacznie szybsza w porównaniu do programów napisanych w \textsl{C/C++}
\end{itemize}
Dodatkowo będzie
W 2012 roku pojawiła się pierwsza wersja języka
