...
Najniższe wyniki wykorzystania procesora zaobserwowano podczas testów aplikacji \textsl{Go}. Średnio podczas testu aplikacja wykorzystywała od 29\% do 47\% procesor. Średnia wykorzystania procesora podczas testów serwerów \textsl{Tomcat} i \textsl{Jetty} było znacznie wyższe. Dla serwera \textsl{Tomcat} wartość średnia wyniosła od 40\% do 78\%, a \textsl{Jetty} od 36\% do 71\% - zależnie od grupy testów. Średnie wykorzystanie procesora podczas testów z 250 klientami było w większości przypadków niższe. Największa różnica była w grupie testów walidujących istnienie kluczy \textsl{API}
...
Z przeprowadzonych testów wynika, że aplikacja w języku \textsl{Go} osiągała najlepsze rezultaty przepustowości jak i czasów odpowiedzi. Niezależnie od liczby klientów, osiągane wyniki nie różniły się między sobą. Na uwagę zasługuje również fakt, że gdy liczba klientów była większa to liczba obsługiwanych żądań w ciągu sekundy nie spadała tak bardzo, mimo iż czasy odpowiedzi żądań były wyraźnie dłuższe.

Aplikacja w języku \textsl{Java} osiągała wyraźnie niższe rezultaty przepustowości, szczególnie podczas testów z 250 klientami. Porównując wyniki testów 100 i 250 klientów można zaobserwować mniejszą liczbę przetworzonych żądań gy z aplikacji korzystało 250 klientów równolegle. Wyraźne były też chwilowe spadki przepustowości. Czasy odpowiedzi również były dłuższe od aplikacji w \textsl{Go}. Analizując wyniki testowanych serwerów, na których była uruchomiona aplikacja \textsl{Java} lepsze rezultaty zaobserwowano na serwerze \textsl{Jetty}. Zarówno wyniki przepustowości oraz czasów odpowiedzi w większości przypadków testowych wykazywały przewagę nad serwerem \textsl{Tomcat}. 

Analiza wykorzystania zasobów maszyny przez poszczególne aplikacje pokazuje, że aplikacja w \textsl{Go} wymagała znacznie mniejsze zasoby od aplikacji w języku \textsl{Java}. Dzięki temu odpowiednio konfigurując środowisko można uruchomić kilka instancji aplikacji \textsl{Go} równolegle na jednej maszynie, by lepiej wykorzystać jej zasoby. Serwer \textsl{Jetty} w większości przypadków testowych, wymagał nieznacznie większych zasobów (procesora i pamięci \textsl{RAM}) od serwera \textsl{Tomcat}

Grupą testów podczas której wyniki najbardziej się wahały były testy operacji \textsl{CRUD}, które najbardziej wykorzystywały komunikację z bazą danych \textsl{MongoDB}. Podczas testów widoczne było mniejsze zużycie procesora na maszynie testowej. Najlepsze rezultaty notowano podczas testów walidacji \textsl{API} oraz obiektów \textsl{Cache}, gdzie komunikacja z bazą sprowadzała się do sprawdzenia, czy poszukiwany dokument istnieje. Walidacja \textsl{API} okazała się najbardziej wykorzystywać zasoby procesora, podczas testów aplikacji w języku \textsl{Java}. 