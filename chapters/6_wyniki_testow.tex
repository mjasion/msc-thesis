...
Z przeprowadzonych testów wynika, że aplikacja w języku \textsl{Go} osiągała najlepsze rezultaty przepustowości jak i czasów odpowiedzi. Wyniki przepustowości, niezależnie od liczby klientów, nie różniły się. Na uwagę zasługuje fakt, że gdy liczba klientów była większa, liczba obsługiwanych żądań w ciągu sekundy nie spadała, mimo iż czasy odpowiedzi żądań były wyraźnie dłuższe, dzięki wykorzystywaniu przez język opisywanych \textsl{go routine}.

Aplikacja w języku \textsl{Java} osiągała wyraźnie niższe rezultaty przepustowości, szczególnie podczas testów z 250 klientami. Porównując wyniki testów 100 i 250 klientów można zaobserwować mniejszą liczbę przetworzonych żądań, wyraźne były też chwilowe spadki przepustowości.  Czasy odpowiedzi również były dłuższe od aplikacji w \textsl{Go}. W niektórych przypadkach testowych były nawet 3 razy dłuższe. 

Analizując wyniki testowanych serwerów, na których była uruchomiona aplikacja \textsl{Java} lepsze rezultaty zaobserwowano na serwerze \textsl{Jetty}. Zarówno wyniki przepustowości oraz czasów odpowiedzi w większości przypadków testowych wykazywały przewagę nad serwerem \textsl{Tomcat}. 

Analiza wykorzystania zasobów maszyny przez poszczególne aplikacje pokazuje, że aplikacja w \textsl{Go} wymagała znacznie mniejsze zasoby od aplikacji w języku \textsl{Java}. Oznacza to, że odpowiednio konfigurując środowisko można uruchomić kilka instancji aplikacji \textsl{Go} równolegle, by lepiej wykorzystać zasoby maszyny, na której jest uruchomiona.  

Grupą testów podczas której wyniki najbardziej się wahały były testy operacji \textsl{CRUD}, które najbardziej wykorzystywały komunikację z bazą danych \textsl{MongoDB}. Podczas testów widoczne było mniejsze zużycie procesora na maszynie testowej. Najlepsze rezultaty notowano podczas testu walidacji \textsl{API}, gdzie komunikacja z bazą sprowadzała się do sprawdzenia, czy poszukiwany klucz istnieje. Ta grupa testów okazała się najbardziej wykorzystywać zasoby procesora, szczególnie podczas testów aplkiacji w języku \textsl{Java}.